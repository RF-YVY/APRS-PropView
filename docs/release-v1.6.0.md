...
- Added browser launch selection for desktop startup behavior. Current builds use blank for system default and `none` for headless or service deployments.
...
- For headless Pi/VM deployments, set `[web].launch_browser = "none"` if no browser should be opened automatically.
